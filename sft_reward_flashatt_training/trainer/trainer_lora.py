...
from .utils.module.lora import convert_linear_layer_to_lora, convert_lora_to_linear_layer, only_optimize_lora_parameters, only_optimize_layer_parameters
...
def unwrap_model(model: nn.Module) -> nn.Module:
    ...
    elif hasattr(model, "rwmodel"):
        return unwrap_model(model.rwmodel)
    elif hasattr(model, "transformer"):
        return unwrap_model(model.transformer)
...
class Trainer:
    ...
    def _get_collator_with_removed_columns(
        ...
        if "labels" not in signature_columns:
            signature_columns.append("labels")
    ...
    def _inner_training_loop(
        ...
        if self.args.lora_dim > 0:
            lora_module_name = self.args.lora_module_name.split(",")
            lora_dim = self.args.lora_dim
            lora_alpha = self.args.lora_alpha
            lora_droppout = self.args.lora_droppout

            self.model = convert_linear_layer_to_lora(self.model, lora_module_name=lora_module_name, lora_dim=lora_dim,
                                                  lora_alpha=lora_alpha, lora_droppout=lora_droppout)

        if self.args.only_optimize_lora:
            self.model = only_optimize_lora_parameters(self.model)

        if self.args.only_optimize_layers is not None:
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            if isinstance(train_dataloader, DataLoader) and isinstance(train_dataloader.sampler, DistributedSampler):
                ...
                logger.info(
                    f"Your dataloader are DistributedSampler, and shuffle in {epoch}"
                )
    ...
    def log(self, logs: Dict[str, float]) -> None:
        ...
        if self.state.epoch is not None:
            logs["epoch"] = round(self.state.epoch, 3)
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        # text = inputs["input_ids"]
        # print(self.tokenizer.decode(text).replace("\n", ""))
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        if not isinstance(self.model, PreTrainedModel):
            if isinstance(unwrap_model(self.model), PreTrainedModel):
                save_state_dict = unwrap_model(self.model).state_dict()

                if self.args.lora_dim > 0:
                    lora_state_dict = {}

                    for name in save_state_dict:
                        if "lora" in name:
                            lora_state_dict[name] = save_state_dict[name]

                    torch.save(lora_state_dict, os.path.join(output_dir, "lora_" + WEIGHTS_NAME))
                else:
                    if self.args.rl_alpha >= 0.:
                        self.model.rwmodel.save_pretrained(output_dir, state_dict=state_dict)
                    else:
                ...
                for name in save_state_dict:
                    if "v_head" in name:
                        rw_state_dict[name] = save_state_dict[name]
                        
                torch.save(rw_state_dict, os.path.join(output_dir, "rw_" + WEIGHTS_NAME))
