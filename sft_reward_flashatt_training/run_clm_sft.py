...
def main():
    ...
    def replace_function(example):
        prompt_input, prompt_no_input, prompt_no_instruct = PROMPT_DICT["prompt_input"], PROMPT_DICT["prompt_no_input"], \
                                                            PROMPT_DICT["prompt_no_instruct"]

        if example.get("instruction", "") != "":
            text = prompt_input.format_map(example) if example.get("input", "") != "" else prompt_no_input.format_map(
                example)
        else:
            text = prompt_no_instruct.format_map(example)
