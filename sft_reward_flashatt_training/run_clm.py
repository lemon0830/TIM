...
from trainer import Trainer, replace_llama_attn_with_flash_attn
...
def main():
    ...
    if "WELM" not in model_args.model_name_or_path:
    ...
    def replace_function(example):
        ...
        example["badouts"] = f"{example['bad_output']}".replace("\\n", "\n")
    ...
    def tokenize_function(examples):
        ...
        for idx, (s, t, b) in enumerate(zip(sources_token["input_ids"], label["input_ids"], badout["input_ids"])):
            Len_s = len(s)
            Len_t = len(t)
            ...
            if Len_bad_input > block_size:
                fix_len_b = Len_bad_input - block_size
                bad_input_ids = bad_input_ids[:-fix_len_b]
                bad_attention_mask = bad_attention_mask[:-fix_len_b]
            else:
                fix_len_b = block_size - Len_bad_input
                bad_input_ids = bad_input_ids[:] + [tokenizer.pad_token_id for _ in range(fix_len_b)]
                bad_attention_mask = bad_attention_mask[:] + [0 for _ in range(fix_len_b)]

            assert len(
                cur_input_ids) == block_size, f"Not match input length {len(cur_input_ids)} and block_size {block_size}, Len_s {Len_s}, Len_t {Len_t}"
            assert len(
                bad_input_ids) == block_size, f"Not match input length {len(bad_input_ids)} and block_size {block_size}"

            output["input_ids"].append(cur_input_ids[:] + bad_input_ids[:])
            output["labels"].append(cur_labels[:] + [IGNORE_IDX for _ in range(block_size)])
            output["attention_mask"].append(cur_attention_mask[:] + bad_attention_mask[:])
